...
export class Pipeline {
  ...
  /**
   * Close out the open game: synthesize `result: *` if it ended without a board
   * result so node-tlcv finalizes + re-arms.
   */
  private closeCurrentGame(): void {
    if (!this.gameActive) return;
    if (!this.resultEmitted) this.sink.emitResult(RESULT_UNKNOWN);
