...
test("server: live clocks/PV don't reach a joining client before its snapshot lands", async () => {
  const { server, port } = await startServer();
  const early = await TestClient.create(port, 'early');
  const late = await TestClient.create(port, 'late');
  const fen1 = 'FEN: rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq';
  try {
    early.logon();
    await waitFor(() => early.received.includes('LOGON SUCCESSFUL'));
    server.setSite('Arena');
    server.setPlayers('Engine W', 'Engine B');
    server.emitMove({
      fenTruncated: 'rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq',
      color: 'w',
    ...
    server.emitClocks(98_000, 97_500);
    server.emitPv('w', 5, 12, 100, 100, ['Nf3']);
    // FMR is the last reliable msg of the move; clocks/PV are unwrapped.
    await waitFor(() => early.received.includes('FMR: 0') && early.received.includes('WPV: 5 12 100 100 Nf3'));

    late.logon();
    // While the snapshot is in flight, live clocks/PV keep flowing with NEW values.
    const live = setInterval(() => {
      server.emitClocks(40_000, 40_000);
      server.emitPv('b', 6, -3, 200, 200, ['Nc6']);
    }, 20);
    await waitFor(() => late.received.includes('WTIME: 40000 otim 40000') && late.received.includes('BPV: 6 -3 200 200 Nc6'), 2000);
    clearInterval(live);

    // The joiner's snapshot clock/PV precede the first live one: no unwrapped
    // update overtook the snapshot's FEN while it was still queued.
    const fenIdx = late.received.indexOf(fen1);
    assert(fenIdx !== -1);
    assert(late.received.indexOf('WTIME: 98000 otim 97500') < late.received.indexOf('WTIME: 40000 otim 40000'));
    assert(late.received.indexOf('WPV: 5 12 100 100 Nf3') < late.received.indexOf('BPV: 6 -3 200 200 Nc6'));
    assert(late.received.indexOf('WTIME: 40000 otim 40000') > fenIdx);
    assert(late.received.indexOf('BPV: 6 -3 200 200 Nc6') > fenIdx);
  } finally {
    early.close();
    late.close();
    server.close();
...
test("server: a move queued before LOGON doesn't reach the joiner ahead of the snapshot", async () => {
  const { server, port } = await startServer();
  const early = await TestClient.create(port, 'early');
  const late = await TestClient.create(port, 'late');
  try {
    ...
    await waitFor(() => early.received.includes('FMR: 0'));
    // Enqueue a move but do NOT wait for it to drain before `late` joins.
    server.emitMove({
      fenTruncated: 'rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq',
      color: 'b',
      fullMoveNumber: 1,
    ...
    // The joiner's first real message is the snapshot's first item — none of
    // the pre-LOGON move's FEN/MOVE/FMR overtook the snapshot for it.
    await waitFor(() => late.received.some((m) => m !== 'LOGON SUCCESSFUL'));
    assert(late.received.find((m) => m !== 'LOGON SUCCESSFUL') === 'SITE: Arena');
  } finally {
    early.close();
    late.close();
    server.close();
...
test('server: a joiner with no game state registers immediately and gets live moves', async () => {
  const { server, port } = await startServer();
  const c = await TestClient.create(port, 'solo');
  try {
    c.logon();
    await waitFor(() => c.received.includes('LOGON SUCCESSFUL'));
    // No snapshot exists, so registration must be immediate — give any
    // (nonexistent) pending window a chance to bite, then play a move.
    ...
    server.emitClocks(1_000, 1_000);
    await waitFor(
      () => c.received.some((m) => m.startsWith('WMOVE')) && c.received.includes('WTIME: 1000 otim 1000'),
    );
  } finally {
    c.close();
    server.close();
